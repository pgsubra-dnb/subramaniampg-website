...
/**
 * Corporate review-credit bundles — three fixed sizes, GST added on top.
 * Nothing self-serve above 500; the buyer emails PGS for more. Requires a
 ...
 *
 * "credit" is the DB unit; the user-facing word is "OKR Review" / "Goal Review"
 * (see lib/okrAllyBrand.ts).
 */
...
export default function CorporateClient({ brand = DEFAULT_BRAND }: { brand?: Brand }) {
  const v = vocab(brand)
  ...
  async function buy() {
    ...
    try {
      ...
      const rzp = new Razorpay({
        ...
        currency: 'INR',
        name: v.product,
        description: `${reviewCount(brand, order.credits)} — company pool`,
        order_id: order.orderId,
        prefill: order.prefill,
        theme: { color: T.primary },
        handler: async (resp: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => {
          const verifyRes = await fetch('/api/okr-ally/verify-payment', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
          ...
          if (vj.success) {
            setMsg({
              kind: 'ok',
              text:
                `Payment confirmed — ${reviewCount(brand, order.credits)} added to ${companyName.trim()}'s pool. ` +
                `${adminEmail.trim()} is now the company admin and can allocate them from the Company tab in ${v.product}. ` +
                (vj.invoiceUnissued
  ...
  return (
    <Page>
      ...
      <p style={{ color: T.muted, fontSize: 14, lineHeight: 1.6, marginBottom: 18 }}>
        Buy a pool of {v.reviews} against your company GSTIN. A designated admin then allocates them
      ...
      {me && !me.authenticated ? (
        <div style={{ background: T.card, border: `1px solid ${T.hairline}`, borderRadius: 12, padding: 18 }}>
          <p style={{ fontSize: 14, color: T.charcoal, margin: 0 }}>
            Sign in to {v.product} first, then come back here.
          </p>
          <div style={{ marginTop: 12 }}>
            <a href={v.path}>
              <Btn>Go to {v.product} sign-in</Btn>
            </a>
          </div>
        </div>
      ) : (
        <>
              ...
              return (
                ...
                >
                  <div style={{ fontFamily: 'var(--font-inter), sans-serif', fontWeight: 600, fontSize: 16 }}>
                    {reviewCount(brand, b.credits)}
                  </div>
                  <div style={{ fontSize: 12.5, opacity: 0.85 }}>{money(Math.round(b.base / b.credits))} each</div>
          ...
          <p style={{ fontSize: 12.5, color: T.muted, marginBottom: 18 }}>
            Need more than 500 {v.reviews}? Email us at{' '}
          ...
          <div style={{ background: T.card, border: `1px solid ${T.hairline}`, borderRadius: 12, padding: 16 }}>
            ...
            <input
              ...
              placeholder="who will allocate them"
            />
            <p style={{ fontSize: 11.5, color: T.muted, margin: '4px 0 0' }}>
              Can be you or a colleague. They get a Company tab in {v.product} to allocate and reclaim them.
            </p>

            <div style={{ marginTop: 14 }}>
              <Btn onClick={buy} disabled={busy || gstinInvalid}>
                {busy ? 'Opening…' : `Buy ${v.reviews}`}
              </Btn>
            </div>
          </div>
          ...
          <p style={{ fontSize: 11.5, color: T.muted, marginTop: 14 }}>
            <a href={v.path} style={{ color: T.emeraldDark, fontWeight: 600 }}>
              ← Back to {v.product}
            </a>
          </p>
        </>
